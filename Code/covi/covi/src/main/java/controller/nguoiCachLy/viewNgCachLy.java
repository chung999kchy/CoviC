...
public class viewNgCachLy extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<NguoiCachLy> list = dao.getList((page - 1)*recordsPerPage, recordsPerPage, khu.getIdKhuCachLy());
        int noOfRecords = dao.getNoOfRecord(khu.getIdKhuCachLy());
        //System.out.println("abc "+noOfRecords);
        int noOfPages = (int) Math.ceil(noOfRecords * 1.0 / recordsPerPage);
        request.setAttribute("noOfRecords", noOfRecords);
        request.setAttribute("ngCachLyList", list);
        request.setAttribute("noOfPages", noOfPages);
        request.setAttribute("currentPage", page);
        RequestDispatcher view = request.getRequestDispatcher("../NguoiCachLy/list.jsp");
